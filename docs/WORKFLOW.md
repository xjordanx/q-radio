...
Newest snapshot tag ON THIS BRANCH   git tag -l 'q-sent/*' --sort=-creatordate --merged | head -1
Newest snapshot tag anywhere         git tag -l 'q-sent/*' --sort=-creatordate | head -1
See tags in context, all branches    git log --oneline --decorate --graph --all
